...
`collector.py` uses ThetaData exclusively for historical quotes, individual trades with matched quotes, open-interest reports, stock end-of-day records, and a dated observed contract universe. Discovery combines quoted/traded contract lists with one bulk OI report per underlying-day, so contracts found only in OI remain eligible; each contract records its discovery sources. Every normal collection run also requests dividends, splits, SOFR and all 11 documented Treasury tenors, and VIX daily and intraday prices. Quotes default to tick events so later frequency comparisons can use the same source records; explicit sampled intervals remain available for exploratory pulls. Downloads are spooled to disk and converted in batches, and stock-reference selection also reads batches. Raw values, request provenance, missing data, and coverage reports are retained, with event times, sample times, and report dates distinguished. Stock references at 10:30, 13:00, and 15:00 New York time select the contracts to download. The combined universe remains an observed list, not proof of complete historical listings. Evaluation samples, waiting-time features, regimes, and pricing inputs are left to later work; these collection settings do not select the paper's final comparison frequencies.
