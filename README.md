...
Full candidate-universe snapshots (dated quote/trade lists and bulk OI) are collected only on enrollment days with underlying-price access. These document why contracts could enter the sample, including quiet OI-only contracts. Between entries, OI is retained only for tracked contracts; EOD uses that retention rule every day. Daily manifests distinguish an `observed`, `incomplete`, or `not_requested` universe, with null counts for the latter two. The `universes` table therefore contains enrollment snapshots, while `contracts` contains daily tracked membership. Missing tracked OI reports appear in `coverage.missing_option_oi_count`; an absent report never becomes an invented zero. [Theta's OI documentation](https://docs.thetadata.us/operations/option_history_open_interest.html) describes these as reports of the previous session's closing open interest.
...
Schema v8 records weekly discovery, tracked OI between entries, and explicit universe and OI coverage status. Existing caches and smoke-test outputs remain untouched and readable; identical compatible raw requests can be reused. Earlier collection manifests cannot satisfy this policy. Entry-window dates and enrollment/retention rules are part of the policy identity; changing worker counts does not change that identity. A failed discovery month must be retried before advancing that root's cohort. Resume checks completed requests and artifacts even when coverage gaps were recorded.
...
The September 12 live check of the earlier v6 daily-enrollment policy used a fresh collector cache for SPY/AAPL on June 2–3, 2025: **61 successful requests, 70,532 saved rows, 24.07 seconds, 33 files, 1.63 MB**. Selected quotes/EOD observations were present; absent option activity bars remained explicit gaps. A recovery check reused saved responses with no downloads, and monthly resume checks passed. This bounded check excluded catalogues, references, and the expiration tail. It does not measure the current weekly policy or forecast full-history runtime/storage. Offline fixtures cover the schedule, retention, and missing OI. Data and measurement reports stay local.

Replaying those saved observations under v7 and v8 with identical weekly selections reduced total stored bytes from **791,448 to 720,528 (9%)**, and Parquet bytes from **529,304 to 454,288 (14%)**. Daily cohort membership and retained price/activity/EOD tables were identical. The replay used previously retained vendor rows, not fresh full-chain downloads; processing took 1.89 versus 2.09 seconds, so it does not demonstrate faster overall collection. Separately, canonical contract-key conversion on 103,488 repeated saved quote rows took a median 0.121 versus 0.036 seconds over seven runs, with identical keys. Distinct strikes/expirations are converted once per batch, retention bounds once per response, and request dates once per collection phase.

Using the saved underlying-date catalogues, weekly discovery reduces the planned quote/trade listing requests for 2017–2025 from **92,718 to 19,264**. This is a 79% reduction in those listing calls, not in total downloads or runtime. Full-history costs still depend on how many cohorts overlap and on Theta throughput. Bulk-versus-individual-strike download speed remains unmeasured; the planner keeps bulk expiration requests. A moving `strike_range` could drop tracked contracts after spot moves. `--output-dir` already supports a local SSD directory outside a synced folder; no storage location was changed or sync overhead measured.
