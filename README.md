...
`collector.py` uses ThetaData exclusively for historical quotes, individual trades with matched quotes, open-interest reports, stock end-of-day records, and a dated observed contract universe. Discovery combines quoted/traded contract lists with one bulk OI report per underlying-day, so contracts found only in OI remain eligible; each contract records its discovery sources. Every normal run also requests dividends, splits, SOFR and all 11 documented Treasury tenors, and VIX daily and intraday prices. Quotes default to tick events; explicit sampled intervals remain available for exploratory pulls.

Normal runs include 60 earlier trading sessions of stock quotes, trades, and EOD records by default. `--lookback-sessions` changes this collection buffer; `0` disables it. Rates and VIX EOD begin at the same earlier date. Corporate-action requests extend through the study's end date plus the maximum selected DTE, so events before an option's expiration are not cut off at the study boundary. Announcement dates and unknown amounts are preserved; later events are not assumed to have been known at an earlier observation. The lookback does not define a calibration window or guarantee continuous histories for the same option contracts.

Responses download to disk and convert to Parquet in batches. Strict CSV checks reject extra/missing fields and malformed records at every batch boundary. Raw values, conditions, row order, duplicates, and separate clock meanings remain available. Session coverage distinguishes successful requests from missing option quotes, stock observations, and selection references. Daily rate/index reports list exchange sessions without a dated observation, including possible differences in publisher holidays. Empty trades, OI, and corporate-action responses are not automatically treated as failures. Older parsed caches that lack strict CSV validation require new downloads; original files remain preserved, and pre-refactor caches with verified CSV bytes can be revalidated.

Stock references at 10:30, 13:00, and 15:00 New York time select contracts to download. The combined universe remains an observed list, not proof of complete historical listings. Evaluation samples, waiting-time features, regimes, and pricing inputs are left to later work; these collection settings do not select the paper's final comparison frequencies.
